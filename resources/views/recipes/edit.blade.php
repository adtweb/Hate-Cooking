                    ...
                    <form method="POST" action="{{ route('recipes.store') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
                        ...
                        <div>
                            ...
                            @foreach(\App\Models\Category::all() as $category)
                                <div class="inline-flex p-5">
                                    <label>
                                        <input type="checkbox" value="{{ $category->id }}" class="mt-1" name="categories[]" @checked(old('categories[$category->id]') or in_array($category->id, $categories)) /> {{ $category->value }}
                                    </label>
                                </div>
                            @endforeach
                        </div>
                        <div>
                            ...
                            @foreach(\App\Models\Quality::all() as $quality)
                                <div class="inline-flex p-5">
                                    <label>
                                        <input type="checkbox" value="{{ $quality->id }}" class="mt-1" name="qualities[]" @checked(old('qualities[$quality->id]') or in_array($quality->id, $qualities)) /> {{ $quality->value }}
                                    </label> {{ $quality->value }}
                                </div>
                            @endforeach
                        </div>
